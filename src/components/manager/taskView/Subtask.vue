<template>
  ...
  hover:bg-main-purple hover:bg-opacity-25" :for="subtask?.title">
    <div class="relative">
      <input @change="check($event, subtask)" class="cursor-pointer " v-model="subtask.isCompleted" type="checkbox" :id="subtask?.title">
...
const check = async (e, subtask) => {
  var data = await axios.post("/api/v1/subtasks/"+ subtask.id + "?isCompleted=" + subtask.isCompleted);
}
