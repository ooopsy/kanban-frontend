...
const onDelete = () => {
  if (managerStore.delete.board) {
    var board = boardsStore.boards.splice(boardsStore.selectedBoard, 1)
    axios.delete("/api/v1/boards/" + board[0].id ,{ withCredentials: true });
    managerStore.hideOverlay()
  } else {
    boardsStore.getCurrentColumn.tasks.splice(boardsStore.selectedTask, 1)
    axios.delete("/api/v1/tasks/" + boardsStore.selectedTaskObj.id ,{ withCredentials: true });
